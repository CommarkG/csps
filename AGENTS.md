...
- ❌ **CHAT TRANSFER — NEW AI RESPONSE FORMAT (mandatory):** When receiving a chat-transfer prompt from the Governor, FIRST response MUST start with:
  `"Hi previous chat, I am the new chat continuing your work."` then ONE of:
  `"All is understood — I have all I need. [brief mandate confirmation]"` OR
  `"Thanks for the files — I have some things to clarify: [specific questions]"`
  No other opening is valid. No summaries. No "I've read the handoff." This format lets the Governor scan alignment in 3 seconds.
