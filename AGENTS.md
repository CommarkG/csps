...
- ❌ **LAYER SEPARATION: AI session mandates are CORE layer work only (libs/, tools/, governance/).** App-layer work (pnpm db:push, dev server testing, feature additions to apps/*) is Governor-triggered — requires Governor credentials + product decisions. Never put app-layer activities in an AI session mandate. Never surface app-layer work as "optimal next step" unless Governor explicitly requests it. (S016 Governor directive — csps-layer-separation.md)
