...
- ❌ **LAYER SEPARATION — concept, not blanket rule:** Never PROACTIVELY include app-layer activities (pnpm db:push, dev server, feature additions to apps/*) in AI session mandates or optimal next steps. WHY: AI-proactive app work displaces core work and creates premature optimization. The Governor directs app-layer work when ready — not the AI. ESCAPE HATCH: If the Governor explicitly requests app-layer work ("let's deploy today", "help me set up .env.local"), execute it fully — the rule governs AI initiation, not AI execution. (S016 — csps-layer-separation.md)
