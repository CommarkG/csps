...
export default function AccountSetupPage() {
  ...
  const handleWizardComplete = async (archetype: string) => {
    // Store archetype in Clerk publicMetadata (PI-001 spec)
    // Note: Clerk updateUser requires server-side — call API route in production
    // For now, redirect to dashboard (archetype stored via Inngest user.created event)
    setPhase('done')
    router.push('/')
  }
