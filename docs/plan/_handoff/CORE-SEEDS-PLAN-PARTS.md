...
## NODE — Naive Personas Doctrine (S072 authored, S072-queued)
**ANCHOR:** Every journey/plan/UI audit invokes ≥1 naive persona (sample-set: 5 user + 4 developer + 2 plan-audit = 11 — expandable per P-META-028) whose attention span + jargon tolerance + mental model match the actual target audience — not the architect's. Closes EXPERT-PERSONA-AUDIT-BIAS: the existing 8 expert personas (cruel-critic / consolidation-expert / bottleneck-expert / balance-expert / schema-expert / ux-expert / synergy-master / vocabulary-canon) audit from architectural altitude; the native AI reviewer (Opus/Sonnet) is itself an expert-tier persona by construction.
**ALIGNS:** [docs/plan/pillar-0-governance/NAIVE-PERSONAS-DOCTRINE.md](docs/plan/pillar-0-governance/NAIVE-PERSONAS-DOCTRINE.md) · extends Facet E selectPersonas() (M4) + 8 existing skill personas + threshold INVOKE class `naive_audit_request` (new 12th class — expansion per cornerstone) + AI-PROFILING (naive findings expose D6/D3/D7 defaults) + PLATFORM-OBSERVATION pipeline (OBSERVE→...→MEASURE-AGAIN) · completes Pillar 7 (frictionless onboarding) + Pillar 8 (developer's journey) via persona-walked audit. NO parallel machinery.
**STATUS:** **DRAFT pending Governor ratification — S072-queued**. Build PE order: L1 author 11 .claude/skills/naive-* SKILL.md (~1h) → L2 extend selectPersonas() + threshold class (~30min) → L3 first audit-pass on existing artifacts (~1h) → L4 weekly-audit + PLATFORM-OBSERVATION absorption (~30min) → L5 apply findings to journey-completion. Total ~3-4h.
