...
## NODE — Naive Personas Doctrine (S072 authored, S073-queued per completion-discipline)
**ANCHOR:** Every journey/plan/UI audit invokes ≥1 naive persona produced by the persona-engine.mjs from 3 sealed L1 archetypes (naive-end-user-core / naive-developer-core / naive-plan-audit-core) tuned along 6 sealed L2 parameter axes (jargon_tolerance / attention_span / domain_familiarity / frustration_threshold / mental_model_template / trust_baseline — mapped directly to AI-PROFILING D1-D13). L3 instances generated on demand · vaulted if useful · K≥3 evidence-driven promotion. Closes EXPERT-PERSONA-AUDIT-BIAS (existing 8 expert personas all audit from architectural altitude; native AI is itself expert-tier by construction).
**ALIGNS:** [docs/plan/pillar-0-governance/NAIVE-PERSONAS-DOCTRINE.md](docs/plan/pillar-0-governance/NAIVE-PERSONAS-DOCTRINE.md) v2 architecture · research vaulted at [docs/plan/_handoff/VAULT/research/S072-naive-personas-research.md](docs/plan/_handoff/VAULT/research/S072-naive-personas-research.md) · composes with **9 existing CSPS abilities** (Facet E selectPersonas + threshold + AI-PROFILING + comms-schema audience_hierarchy + vocabulary glossary + ai-behavior-signals + PLATFORM-OBSERVATION + CIP RIPPLE-QC + weekly-persona-audit) — NO parallel machinery · core-spine GVRN>VALD>ARCH>AI>OPER precedence applied (L1 GVRN-sealed · L2 VALD-sealed · L3 ARCH-tunable · engine OPER).
**STATUS:** **DRAFT — vlt-S073-naive-personas-doctrine (NOT S072 per Governor S072 Turn 4 completion-discipline)**. S072 stays focused on CIP + boundary-prompt validator + Q1/Q2/Q4 ratifications. S073 build PE order: L1 author 3 archetype SKILL.md + persona-engine.mjs → L2 extend selectPersonas + 15th threshold class → L3 first audit pass → L4 weekly-audit + PLATFORM-OBSERVATION absorption → L5 apply to Pillar 7+8 journey-completion → L6 `/platform/personas` page. Total ~3.5-4.5h.
