...
| `DUAL_FOCAL_PLAN` | Planning a new CSPS app — triggered before any app implementation | N/A (feeds validate-plan-readiness.mjs via unified-plan.yaml PMI fields) | [`tools/templates/dual-focal-plan.template.yaml`](../../../../tools/templates/dual-focal-plan.template.yaml) | **LIVE (S045 PROTO-036 — authored; outward+inward simultaneous planning; PMI 5-field schema; PROMOTION CHECKLIST embedded)** |
| `DISPATCHER-PATTERN` | Adding a new Write hook — use dispatch-registry.yaml, never settings.json | N/A (structural pattern) | [`.claude/hooks/dispatch-registry.yaml`](../../../../.claude/hooks/dispatch-registry.yaml) | **LIVE (S048 — one settings.json entry is permanent; all new Write hooks go to dispatch-registry.yaml only; eliminates .claude/settings.json permission prompts)** |
| `THIN-READER-PATTERN` | Creating or modifying a .claude/*.sh hook that has detection logic | N/A (structural pattern) | [`tools/config/caq-patterns.yaml`](../../../../tools/config/caq-patterns.yaml) (example) | **LIVE (S048 — .claude/.sh = mechanism only; logic in tools/config/*.yaml; editing YAML never triggers .claude/ permission prompt)** |
