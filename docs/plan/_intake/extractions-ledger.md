...
| EXT-20260505-004 | 2026-05-05T05:00Z | AI_OTHER | low | clean | (parent of 4 sub-IDs A-D) | (parent — see children + governance/depth-discipline) | triaged | sub-IDs A-D routed; CSP series CLOSED with this file; pivot to Phase 6 next | **CSP series #4 (FINAL) — DEPTH_LEVELS_AND_BUNDLING_ORCHESTRATOR_REPORT_ON_CSP.** 14-section report on depth-level discipline + bundling orchestrator wiring + composition with SCHEMA/Core Spines + 9 concrete improvements ranked by leverage × effort. **CRITICAL FOR PHASE 6-10 EXECUTION** — directly informs energy-optimization mechanism for remaining token-optimization phases. **CSP empirical estimate (ESTIMATED): 60-75% reduction on reference-read cost per chat = ~150-300K tokens saved per chat at Opus pricing.** CSPS measurement obligation at Phase 9 close before propagating any number. CSP DNA export series CLOSED — 4 reports together describe complete operational governance. |
| EXT-20260505-004-A | 2026-05-05T05:05Z | AI_OTHER | low | clean | governance/depth-discipline | docs/plan/pillar-1-architecture-and-stack/depth-discipline.md (NEW LEAF; S009 PCR) + frontmatter-standard.md amendment + frontmatter-closed-enums.md extension | routed | 🔥 HIGH — engrave S009 BEFORE Phase 6 spawn templates | 4 distinct depth-level semantics (file_depth_markers / depth_levels_invoked DNA / audit depth L1-SCSE-L2-L3 / depth_tier_authored) + CSPS native 5th (depth_chosen ∈ {3,4,5} for topic-plans) = 5 CSPS depth semantics needing disambiguation BEFORE Phase 6. Unified principle: per-artifact + per-session-load coordination. |
| EXT-20260505-004-B | 2026-05-05T05:05Z | AI_OTHER | low | clean | governance/depth-discipline | tools/templates/governed-artifact-frontmatter.template.md (NEW; CSPS analog of CSP CD-097) + .claude/hooks/pre-tool-use-depth-marker-creation-gate.sh (NEW stub) + B_TEMPLATE_FIRST_CREATION composition | routed | 🔥 HIGH — atomic with Extract A engraving S009 | Mechanical-creation discipline 5-step process: template at creation / pre-push hook check / placeholder values allowed (TBD-S{N} pattern) / validator catches stale placeholders >5 sessions / orchestrator consumes real values. **Solves chicken-and-egg:** "structure not stable" → declare placeholders; mechanical validator forces backfill. |
| EXT-20260505-004-C | 2026-05-05T05:05Z | AI_OTHER | low | clean | governance/depth-discipline | tools/pe-compute.mjs (CSPS analog; per EXT-001-C deep-dive) + topic-plan §6 priority_engine extension + tools/pe-context-cache.json (NEW; cross-session L1 cache) | routed | 🔥 EXCEPTIONAL — schedule Phase 9 (after data accumulates from Extracts A+B) | PE.read_budget extension wires bundling orchestrator: PE consumes file_depth_markers from referenced artifacts; computes read_budget per work item; recommends L1-only / L2 / L3 strategy based on blast level + bundles co-located reads + caches L1 across sessions. **Foundation order: Extract A semantics → Extract B creation gate populates data → Extract C orchestrator consumes.** |
| EXT-20260505-004-D | 2026-05-05T05:05Z | AI_OTHER | low | clean | governance/depth-discipline + cross-cutting (ARCH/GVRN/OPER/VALD all spines) | 9 improvements directly map to Phase 6-10 work + composition with EXT-001-A/C + EXT-002-A/B + EXT-003-C | routed | 🔥 EXCEPTIONAL — THE Phase 6-10 ROADMAP CSP-INFORMED | 9 concrete improvements ranked by leverage × effort: #1 frontmatter template / #2 disambiguate semantics / #3 backfill top offenders / #4 pre-push depth hook / #5 PE.read_budget / #6 HUB-per-spine (CSPS L1_CORE files ALREADY EXIST as equivalent — just add markers) / #7 Supabase columns DEFER / #8 corespine_layer_compliance extension / #9 depth_marker_creation_template_validator. CSPS-adapted 4-phase plan maps to Phase 5+/6/7/8/9/10 of token-optimization. |
