...
| **B_AGENT_ALIGNMENT_PROTOCOL** (S005 turn 25 — every agent passes AAP before invocation; no wildcards) | agent-alignment-protocol.md (the dashboard leaf — 9-check spec + per-class table + alignment-preamble template) + new SKILL.md/agent.zmodel frontmatter fields (`csps_aligned` / `aap_version` / `acknowledged_contracts` / `respects_quality_gates` / `output_contract` / `trust_tier` / `eval_baseline`) + principles.yaml#P-META-010.config | `agent-alignment-coverage` (PR-blocking error — Class A frontmatter check) + `subagent-spawn-preamble-required` (PR-blocking warn — Class B preamble check) — atomic-registered per FSE amendment; planned week-4 | `.claude/hooks/pre-tool-use-agent-aap.sh` (PreToolUse — Class B preamble injection; planned week-4) | `feedback_agent_alignment_protocol.md` (NEW S005 turn 25) + composes with `feedback_validate_before_assume.md` + `feedback_no_invention_without_precedent.md` | behavioral-contracts.md § B_AGENT_ALIGNMENT_PROTOCOL (NEW S005 turn 25) + AGENTS.md hard NO (NEW S005 turn 25 — no-wildcards) + principles.yaml#P-META-010 + agent-alignment-protocol.md dashboard + tools/generators/skill template (week-3+) + Mastra BaseAgent runtime (week-6+) | 5/5 declared; 3/5 mechanical (memory + contract + schema active S005 turn 25; 2 validators + 1 hook + generator-template + runtime deferred per build-order weeks 4-6 with atomic registration per FSE amendment) |
...
| **B_STRUCTURAL_PREVENTION_DISCIPLINE** (S006 turn 8 Q-2 tweak — enhance system constantly; never patch instance) | closing-summary-template.md §10.0j mandatory header (per-proposal schema + zero_proposals_declaration discipline when empty) | `enhancement-proposal-coverage` (per-session error — every skipped/late/partial enforcement detected has paired proposal) + `structural-fix-vs-instance-fix-discipline` (per-session warn — K=2 recurring patterns produce structural fix not instance patch) — atomic per FSE; week-4 impl | `.claude/hooks/post-stop-enhancement-scan.sh` (week-4 — scans session log for enforcement gaps + verifies §10.0j coverage) | `feedback_structural_prevention_discipline.md` (NEW S006 turn 8) | behavioral-contracts.md § B_STRUCTURAL_PREVENTION_DISCIPLINE (NEW S006 L2b) + AGENTS.md hard NO + principles.yaml#P-META-019 + element-reviews place | 5/5 declared; 3/5 mechanical (memory + contract + schema active S006 L1-L2b; 2 validators + 1 hook deferred week-4 with atomic registration per FSE amendment) |
