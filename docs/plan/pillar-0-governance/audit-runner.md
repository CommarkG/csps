...
| `open-plan-levels` | per-session | warn | NEW S014 Phase 3A: surfaces ALL unchecked exit criteria (not just file-path ones) across active topic-plans. Per P-META-020 context-depth-degradation pattern — open plan levels are obligations, not options. Root cause fix for foundation-slices L3 orphaning (plan-promise-abandonment). **Registered S014 Phase 3A atomically. Build ACTIVE (validate-open-plan-levels.mjs in pnpm verify).** | P-META-020 + B_STRUCTURAL_PREVENTION_DISCIPLINE |
