...
| `model-routing-profile-consistency` | PR + per-session | warn | NEW S010 amendment: verifies settings.json `"model"` field matches the `main_session_default.value` of the active profile (`is_default: true`) in [model-routing-dashboard.md §10](./model-routing-dashboard.md#10--configuration-profiles-template-groups). Catches drift between "settings.json says Sonnet" and "dashboard profile says Opus" or vice versa. Also checks T4 per-session model-budget plan cites the active profile_id. Per B_TOKEN_BUDGET R2 + B_COGNITIVE_CONTEXT_DISCIPLINE + model-routing-dashboard.md §10. **Registered S010 atomically. Build deferred week-4 S012 (ships alongside Phase 8 bundling orchestrator as part of full auto-tiering enforcement).** |
