...
enforcement_trio:
  t1:
    tier: hook
    path: ".claude/hooks/pre-tool-use-nominal-stop-detector.sh"
    status: advisory
  t2:
    tier: validator
    path: "tools/validators/validate-no-nominal-stops-mid-milestone.mjs"
    status: advisory
  t3:
    tier: memory
    path: "tools/scripts/session-open-context.mjs (LONG-RUN injection) + AGENTS.md hard-NO B_LONG_RUN_BUILDER_DOCTRINE"
    status: active
  exempt_reason: null
  superseded_by: "csps.governance.long-run-builder-doctrine"
  m07_fix: "S071 M0.7 — exempt_reason was disproven by post-stop-banned-phrase.sh prior art. T1+T2 built; T3 upgraded from claimed to actually-injected."
---
