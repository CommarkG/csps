...
enforcement_trio:
  ...
  t3:
    tier: memory
    path: "** `session-open.sh` (active) + AGENTS.md hard rule (queued — PARK-S089-AGENTS-HARD-RULES, edit via principles.yaml + codegen, not directly)"
    status: active
...
[//]: # (Phase 7 token-optimization §9.8 Candidate #2 S010 — load B_CHALLENGE_ON_MERIT.md instead of full ~48K monolith)
