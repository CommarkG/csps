...
enforcement_trio:
  t1:
    tier: hook
    path: ".claude/hooks/post-stop-rzf-reminder.sh"
    status: stub
  t2:
    tier: validator
    path: "tools/validators/validate-directive-has-rzf.mjs"
    status: active
  t3:
    tier: memory
    path: "session-open ZF mandate, permanence: high-partial → target: high-full with validate-nominal-rzf.mjs BLOCKING"
    status: active
  exempt_reason: null
---
[//]: # (B_RZF.md)
