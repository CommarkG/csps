...
enforcement_trio:
  ...
  t3:
    ...
    status: active
  exempt_reason: "validate-token-budget.mjs measures usage but cannot mechanically constrain AI response length — budget discipline is reasoning, not pattern-checkable. T3 (session-open token budget mandate + validator advisory) is the only viable tier. (Q3 Opus ruling S062)"
---
[//]: # (B_TOKEN_BUDGET.md)
