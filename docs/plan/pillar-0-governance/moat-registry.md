...
| M-47 | **FRONT-END FUNCTIONAL COMPLETENESS (S086, Governor+Opus emphatic)** | Every CSPS-created page is functionally complete: every interactive element (button, link, tab, form field, toggle, dropdown, handler, data-fetch, toolkit/widget) is enumerated BY CODE and each one verified wired + working. A page with ONE dead element cannot ship green. "Renders = done" is a satisfaction-point trap this kills — the machine forces per-element evidence. MOAT: other platforms ship partial/broken UIs and users waste hours discovering rot; CSPS pages are complete-by-construction. Applies to ALL page-creation paths (users, developers, internal). Dead elements caught on first run S086: `onClick={undefined}` in profiles/developers, 2 dead nav links in TopNav, classification-training missing fetch timeout. Machine also block-tests itself: a deliberate fixture with 3 dead elements must make verify FAIL (meta-verification). | `validate-page-completeness.mjs` (EXTENDED) + `B_FRONT_END_COMPLETENESS` behavioral contract + AGENTS.md hard-NO ("no page ships with a dead element") + page scaffold/template requires element manifest | Per page creation + --extended verify | active-S086 |
