...
| OPEN-061 | council-state.json has no opus_instance field — stale from S023. Every new Opus tab proceeds by convention ("I am OPUS-N") not by data. Add opus_instance field that increments at every new Opus tab open. Wires into session-open.sh to inject correct instance number. | S045 | pending | Opus instance tracking | none yet |
| OPEN-062 | [S3] T1 hook for sonnet-turn.md writes — pre-tool-use hook fires when Sonnet writes to tools/council/sonnet-turn.md and checks: (a) new section starts with "# Sonnet Report —" or "# Sonnet Session" header, (b) first non-header content line is "Opus, this is Sonnet.". ADVISORY. Training default: "AI output is chat-first; writing to file is secondary." Satisfaction point: "Governor can see it in chat." Fix: T1 makes writing correctly mechanical, writing incorrectly visible. Completes Rule 13 enforcement to T1+T2+T3. | S046 | pending | communication protocol T1 | none yet |
