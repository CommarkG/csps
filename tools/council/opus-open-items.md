...
| OPEN-037 | Sacred parts protection — pre-commit hook blocks edits to layout.tsx + middleware.ts + next.config.js without explicit Governor ratification (PROTO directive SHA). PCR approved, awaiting implementation. | S040 | pending | external UX stability | none yet |
| OPEN-038 | Deduplication audit — scan all apps/* + tools/ + docs/ for parallel structures created in S040 and verify alignment. Governor-requested. | S040 | pending | consolidation discipline | none yet |
