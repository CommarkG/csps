...
// ── Active model declaration (S089 B_MODEL_DECLARATION) ─────────────────────
// Read the configured model from .claude/settings.json so Claude can self-declare it.
// MANDATORY: Claude's FIRST response in every new tab must be the model card.
// Governor cannot know the active model from the UI alone — self-declaration is the gate.
let configuredModel = 'UNKNOWN — check .claude/settings.json';
let configuredModelShort = 'UNKNOWN';
try {
  ...
  if (existsSync(settingsPath)) {
    ...
    if (s.model) {
      ...
      configuredModelShort = MODEL_LABELS[s.model] || s.model;
    }
  }
} catch { /* non-fatal */ }

// ── OneClick session resume (S089 B_ONECLICK_FRESHNESS) ──────────────────────
// Read the paste block from .csps/oneclick.md (auto-generated by tools/generate-oneclick.mjs).
// Injected here so Claude receives it as part of additionalContext on every new tab.
// G5 permanence fix: context survives compaction — machine-generated from git state.
let oneclickBlock = '';
try {
  const oneclickPath = join(ROOT, '.csps/oneclick.md');
  if (existsSync(oneclickPath)) {
    const raw = readFileSync(oneclickPath, 'utf8');
    const m = raw.match(/```\n([\s\S]*?)\n```/);
    if (m) oneclickBlock = m[1].trim();
  }
} catch { /* non-fatal — will skip injection */ }
...
const context = [
  // ── B_MODEL_DECLARATION (S089): mandatory first-response gate ─────────────
  // Governor cannot verify the active model from the UI without manually opening the selector.
  // Self-declaration is the only reliable signal. This block must be the FIRST thing Claude outputs.
  // DO NOT skip or defer. DO NOT present session context before this is confirmed.
  '╔══════════════════════════════════════════════════════════════════╗',
  '║        ⚠  MODEL DECLARATION REQUIRED — FIRST RESPONSE ONLY  ⚠  ║',
  '╠══════════════════════════════════════════════════════════════════╣',
  `║  CONFIGURED : ${configuredModelShort.padEnd(50)}║`,
  `║  MODEL ID   : ${configuredModel.padEnd(50)}║`,
  '╠══════════════════════════════════════════════════════════════════╣',
  '║  YOUR FIRST RESPONSE — output ONLY this, nothing else:          ║',
  '║    ACTIVE MODEL: [your actual model name]                        ║',
  '║    ROLE: [Sonnet=builder | Opus=director]                        ║',
  '║    [mismatch note if actual ≠ configured above]                  ║',
  '║    Awaiting Governor confirmation.                               ║',
  '╠══════════════════════════════════════════════════════════════════╣',
  '║  GOVERNOR OPTIONS:                                               ║',
  '║  "confirmed"    → proceed with full session context              ║',
  '║  "use opus"     → type /model → select Opus 4.8 → open new tab  ║',
  '║                   (same hook fires; Opus sees this same card)    ║',
  '║  "use haiku"    → Haiku is sub-agent ONLY (never a UI tab).      ║',
  '║                   Spawn via Agent tool: subagent_type:"haiku-    ║',
  '║                   scout", model:"haiku". Pass context in prompt. ║',
  '╠══════════════════════════════════════════════════════════════════╣',
  '║  ARCHITECTURE DOC (for Opus review + improvement):              ║',
  '║  .csps/session-startup-architecture.md                          ║',
  '╚══════════════════════════════════════════════════════════════════╝',
  '',
  '╔══════════════════════════════════════════════════════════════════╗',
  ...
  '',
  // ── B_ONECLICK_FRESHNESS (S089): session resume state injected automatically ──
  // Source: .csps/oneclick.md (auto-generated by tools/generate-oneclick.mjs after every clean verify).
  // Governor opens a new tab → this appears in Claude context automatically. No paste required.
  oneclickBlock ? '── SESSION RESUME STATE (auto-injected by B_ONECLICK_FRESHNESS) ──' : null,
  oneclickBlock || null,
  oneclickBlock ? '─────────────────────────────────────────────────────────────────' : null,
  '',
  '═══════════════════════════════════════════════════════════════════',
